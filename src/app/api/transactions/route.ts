import { and, eq, gte, lte, ilike, or, desc, asc, count, type SQL } from "drizzle-orm";
import { db } from "@/db";
...
export const GET = route(async (req: Request) => {
  ...
  const payeeId = q.get("payeeId");
  if (payeeId) filters.push(eq(transactions.payeeId, payeeId));
