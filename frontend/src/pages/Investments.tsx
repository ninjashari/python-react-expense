...
const INVESTMENT_RELATED_CATEGORIES = [
  'interest',
  'demat account',
  'recurring deposit',
  'rd return',
...
const Investments: React.FC = () => {
  ...
  const isInvestmentCategory = (name: string): boolean => {
    ...
    return INVESTMENT_RELATED_CATEGORIES.some(cat => lower === cat);
  };
  ...
  const investmentCategoryIds = useMemo(
    () => new Set(investmentCategories.map(c => c.id)),
  ...
  const { data: transactionsData, isLoading: txLoading } = useQuery({
    ...
    queryFn: () => {
      const selectedCategoryIds = filters.categoryIds.length > 0
        ? filters.categoryIds.map(c => c.value).join(',')
        : Array.from(investmentCategoryIds).join(',');

      return transactionsApi.getAll({
        account_ids: accountIdsParam,
        category_ids: selectedCategoryIds || undefined,
        transaction_type: filters.transactionType || undefined,
        start_date: filters.startDate,
        end_date: filters.endDate,
        page: page + 1,
    ...
    enabled: effectiveAccountIds.length > 0 && investmentCategoryIds.size > 0,
  });
  ...
  const { incomeCategories, expenseCategories, categoryGrandTotal } = useMemo(() => {
    ...
    const filtered = categoryData.filter((c: any) => investmentCategoryIds.has(c.id));
    const income = filtered.filter((c: any) => c.income > 0).sort((a: any, b: any) => b.income - a.income);
    const expense = filtered.filter((c: any) => c.expense > 0).sort((a: any, b: any) => Math.abs(b.expense) - Math.abs(a.expense));
    const grand =
  ...
  }, [categoryData, investmentCategoryIds]);
