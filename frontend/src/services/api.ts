...
export const rewardPointsApi = {
  getAll: (): Promise<RewardPointRedemption[]> =>
    api.get('/reward-points/').then(res => res.data),

  getSummary: (): Promise<{ items: RewardPointsSummaryItem[] }> =>
    api.get('/reward-points/summary').then(res => res.data),

  create: (data: CreateRewardPointRedemptionDto): Promise<RewardPointRedemption> =>
    api.post('/reward-points/', data).then(res => res.data),

  update: (id: string, data: Partial<CreateRewardPointRedemptionDto>): Promise<RewardPointRedemption> =>
    api.put(`/reward-points/${id}`, data).then(res => res.data),
